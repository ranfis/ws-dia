 ...
 * Time: 10:55 PM
 */
class EstatusAplicacion extends Model{
    const ESTATUS_APP_RECHAZADA     = 1;
    const ESTATUS_APP_EN_REVISION   = 2;
    const ESTATUS_APP_ACEPTADA      = 3;

    const QUERY_FIND = "SELECT id_estado_aplicacion, descripcion, estatus FROM estado_aplicacion";
    ...
    */
    ...
    public static function find($id = null){
        ...
        if ($id) {
            $query .=" AND id_estado_aplicacion=?";
    ...
    private static function mappingFromDBResult(&$result){
        ...
        $results = [];
        while($result->fetch()){
            $estatusApp = new EstatusAplicacion($bindResult['id'],$bindResult['descripcion']);
            $estatusApp->estatus = new Estatus($bindResult['estatus']);
            $results[] = $estatusApp;
        }
        return $results;
    }
